...
`~/.youmind/config.yaml` 现在只需要配置 YouMind API Key：
...
命令会按 `~/.youmind/config/youmind-ghost-article.yaml` -> `~/.youmind/config.yaml` 的顺序读取 `youmind.api_key` 和 `youmind.base_url`。
...
检查 `~/.youmind/config.yaml` 里的 `youmind.api_key`。这个 skill 现在只通过 YouMind 鉴权。

**Q: 发布时提示 Ghost 没有连接**

先去 YouMind 里连接 Ghost。Ghost 的站点地址和 Admin API Key 现在保存在 YouMind，不在 `~/.youmind/config.yaml`。
