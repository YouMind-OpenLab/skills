 ...
 *
 * 端点契约（apps/youapi spec 016 v3）：26 个 per-op 端点已合并为 6 个 resource 端点
 *   POST /openapi/v1/publishing/{connections|posts|media|engagement|taxonomy|insights}
 *   body: { platform: 'wordpress', action, [actionKey]: payload }（insights 无 action，payload 平铺）。
 *   本层 callPublishing 仍接收旧 op 名，经 buildPublishingRequest 重塑成新 body。
 */
