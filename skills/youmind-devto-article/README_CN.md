...
命令会按 `~/.youmind/config/youmind-devto-article.yaml` -> `~/.youmind/config.yaml` 的顺序读取 `youmind.api_key` 和 `youmind.base_url`。
...
先检查 `~/.youmind/config.yaml` 里的 `youmind.api_key`。现在 skill 只用 YouMind API Key 做鉴权。

**Q: 发布时报 Dev.to 未连接**

先去 YouMind 里完成 Dev.to 账号连接。Dev.to token 保存在 YouMind，不在 `~/.youmind/config.yaml`。
