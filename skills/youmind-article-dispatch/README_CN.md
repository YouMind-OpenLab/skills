...
| Tumblr | youmind-tumblr-article | Connector 中 OAuth 绑定 |
| Ghost | youmind-ghost-article | Connector 中填 站点URL + Admin API Key |
| Beehiiv | youmind-beehiiv-article | Connector 中填 API Key + Publication ID |
| Kit | youmind-kit-article | Connector 中填 API Key |
...
- **多平台**：列出平台名，如 `发到 Dev.to、Ghost 和 X`
...
- **`youmind-devto-article`**：技术深度、代码示例、TL;DR 开头
- **`youmind-hashnode-article`**：更像开发者博客，适合系列化、叙事型技术长文
- **`youmind-wordpress-article`**：HTML CMS 发布，关注分类/标签/摘要，适合 SEO 长文
- **`youmind-ghost-article`**：编辑级 Newsletter 内容，会员分层，邮件优先
- **`youmind-tumblr-article`**：强调作者声音的长文博客体，适合 build log、创作者随笔、轻博客分发
- **`youmind-beehiiv-article`**：Newsletter 原生长文、增长导向标题、兼顾 web + email
- **`youmind-kit-article`**：创作者广播体、subject + preview text 强约束、feed 可读性优先
- **`youmind-wechat-article`**：中文本地化、公众号排版、配图，且支持现成稿件直接排版发送
- **`youmind-x-article`**：精炼为推文串、话题标签
- **`youmind-qiita-article`**：面向日本开发者、GFM Markdown、环境信息、笔记框
