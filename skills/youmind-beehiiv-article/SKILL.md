...
>
> **Important:** Beehiiv post creation can return `403` if the target publication does not have Send API access enabled on Beehiiv's side. Beehiiv 官方文档目前还把 `update post` 标成 `beta / Enterprise`.
...
- List available templates: `List my Beehiiv post templates`
...
5. When the user wants a specific Beehiiv layout, fetch `listPostTemplates` first and use `postTemplateId`.
6. End publish actions with the Beehiiv web URL when available.
