...
运行内置的 OAuth 辅助脚本，自动完成授权、换取 Token、获取 Person URN 并写入 `~/.youmind/config/youmind-linkedin-article.yaml`：
...
6. 将所有凭证写入 `~/.youmind/config/youmind-linkedin-article.yaml`
...
共享的 YouMind 凭证写在 `~/.youmind/config.yaml`，LinkedIn 专属凭证写在 `~/.youmind/config/youmind-linkedin-article.yaml`：
