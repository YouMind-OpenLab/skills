...
`config.yaml` 只需要填写 YouMind API Key：
...
命令从本地 `config.yaml` 读取 `youmind.api_key` 和 `youmind.base_url`。文档和 example 中保持 `https://youmind.com/openapi/v1`。需要对接本地 `youapi` 时，只改本地 `config.yaml` 即可。
...
检查 `config.yaml` 里的 `youmind.api_key`。本 Skill 现在只通过 YouMind 鉴权。

**Q: 发布时提示 X 没连接**

请先在 YouMind 中连接 X 账号。X 的 access token / refresh token 保存在 YouMind 后端，不在 `config.yaml`。
